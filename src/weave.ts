...
// (bw.index: passages only, nothing rides in a prompt), kept current on save, and searched by the tools and the
...
export class Weave implements vscode.WebviewViewProvider, vscode.Disposable {
  ...
  async indexFolder(folder: vscode.Uri, quiet = false, force = false): Promise<{ files: number; passages: number }> {
    ...
    const exclude = extra.length ? `{${[EXCLUDE, ...extra].join(",")}}` : EXCLUDE;
    const files = await vscode.workspace.findFiles(new vscode.RelativePattern(folder, "**/*"), exclude);
    const list = files.filter((u) => (TEXT_EXT.test(u.fsPath) || /(^|\/)(Dockerfile|Makefile|README|LICENSE|CHANGELOG)[^/]*$/i.test(u.fsPath)) && !ARTIFACT.test(u.fsPath));
  ...
  private onSave(d: vscode.TextDocument): void {
    const folder = this.folders.find((f) => d.uri.fsPath.startsWith(f + path.sep));
    if (!folder || !this.isReady || !(TEXT_EXT.test(d.uri.fsPath)) || ARTIFACT.test(d.uri.fsPath)) return;
    clearTimeout(this.saveTimers.get(d.uri.fsPath));
    this.saveTimers.set(d.uri.fsPath, setTimeout(() => { this.indexFile(d.uri, vscode.Uri.file(folder), true).then((r) => { this.out.appendLine(`[weave] ${this.sourceName(folder, d.uri.fsPath)}: ${r.n} passages (saved)`); this.scheduleSave(); }).catch((e) => this.out.appendLine(`weave save: ${e.message || e}`)); }, 1500));
  }
